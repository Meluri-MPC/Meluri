...
export default async function DocsPage() {
  ...
  return (
    <div className="animate-fadeIn">
      ...
      <p className="text-white/60 mb-8">Integrate VelumX MPC into your Stacks dApp in minutes</p>
...
console.log(stx); // "10.5"`}</CodeBlock>
  ...
  recipient: 'SP2A8G...',
        ...
        <Section title="Session Keys (save costs)">
          <p className="text-white/60 text-sm mb-3">Create a session key to sign multiple transactions without calling Turnkey each time. Saves ~$0.005 per transaction.</p>
...
await velumx.logout();`}</CodeBlock>
        ...
        <Section title="Batched Transactions">
          <p className="text-white/60 text-sm mb-3">Send multiple transactions in one API call.</p>
        ...
        <Section title="Troubleshooting">
          <div className="space-y-4 text-sm">
            <div>
              ...
              <p className="text-white/60">You called a wallet method before authenticating. Call <code className="text-[#007aff] bg-white/[0.06] px-1 rounded">await velumx.login()</code> first.</p>
            </div>
            <div>
              <h3 className="font-semibold text-amber-400 mb-1">"Clerk not loaded"</h3>
              <p className="text-white/60">Ensure your app is wrapped with <code className="text-[#007aff] bg-white/[0.06] px-1 rounded">{`<ClerkProvider>`}</code>. VelumX uses Clerk for authentication.</p>
            </div>
            <div>
              <h3 className="font-semibold text-amber-400 mb-1">"Could not determine organization ID"</h3>
              <p className="text-white/60">Your MPC is not provisioned. Go to <a href="/mpc" className="text-[#007aff] hover:underline">MPC Config</a> and click "Provision MPC Infrastructure".</p>
            </div>
            <div>
              <h3 className="font-semibold text-amber-400 mb-1">Rate limit (429)</h3>
              <p className="text-white/60">You exceeded 100 requests/minute. Implement exponential backoff or batch your requests.</p>
            </div>
            <div>
              <h3 className="font-semibold text-amber-400 mb-1">"Wallet not found" (401 on /tx/send)</h3>
              <p className="text-white/60">The sender address does not belong to your organization. Verify the wallet was created with your API key and that the address is correct.</p>
            </div>
            <div>
              <h3 className="font-semibold text-amber-400 mb-1">TypeScript errors with React subpath</h3>
              <p className="text-white/60">Make sure your <code className="text-[#007aff] bg-white/[0.06] px-1 rounded">tsconfig.json</code> has <code className="text-[#007aff] bg-white/[0.06] px-1 rounded">"moduleResolution": "bundler"</code> for subpath exports to work.</p>
            </div>
          </div>
        </Section>
      </div>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-[#0d0d0d] border border-white/[0.08] rounded-[14px] p-6">
...
function CodeBlock({ children }: { children: string }) {
  return (
    <pre className="bg-black border border-white/[0.08] rounded-lg p-4 text-sm font-mono text-white/60 overflow-x-auto whitespace-pre-wrap">
      {children}
    </pre>
  );
}
